...
const Session_8 = () => {
  ...
  const handleScroll = () => {
    setShowButton(document.documentElement.scrollTop > 0 ? true : false);
  };
  useEffect(() => {
  ...
  const [countingText, setCountingText] = useState(0);
  const handleTypeText = (e) => {
    setInputText(e.target.value);
    setCountingText(
      e.target.value === "" ? 0 : inputText.trim().split().length
    );
  };
    ...
    <div>
      <div>
        ...
        <div>
          ...
          resemblance to sections 1.10.3233 of Cicero's work, with the most
          notable passage excerpted below: “Neque porro quisquam est, qui
          dolorem ipsum quia dolor sit amet, consectetur, adipisci velit, sed
          quia non numquam eius modi tempora incidunt ut labore et dolore magnam
          aliquam quaerat voluptatem.” A 1914 English translation by Harris
        ...
        {showButton && (
          <button className="btn-to-top" onClick={handleClickBtn}>
            To Top
          </button>
        )}
      </div>
      <div>
        ...
        />
        <p>The number of word in input filed: {countingText}</p>
      </div>
    </div>
  );
};

export default Session_8;
